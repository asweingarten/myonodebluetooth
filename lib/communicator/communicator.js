...
"use strict"
const serires = require('async/series');
...
class Communicator{
    ...
    initStart(callback) {
        ...
        let commandPayload = this.serialise.command_set_mode(false, false, true);
        ...
        this.peripheral.discoverServices([this.protocol.services.control.id,this.protocol.services.classifier.id, this.protocol.services.imuData.id, this.protocol.services.emgData.id, this.protocol.services.battery.id], (error, services) => {
            ...
            this.commandService.discoverCharacteristics([this.protocol.services.control.COMMAND, this.protocol.services.control.FIRMWARE_VERSION, this.protocol.services.control.MYO_INFO], (error, characteristics) => {
                if(characteristics.length == 3){
                  ...
                  this.commandChar.write(commandPayload, true, (error) => {
                    if(!error){
                      console.log('commandChar set');
                      this.commandChar.write(unlockPayload,true, (error) => {
                        if(!error){
            ...
            series([
              (seriesCallback) => {
                this.batteryService.discoverCharacteristics([this.protocol.services.battery.BATTERY_LEVEL], (error, characteristics) => {
                    if(characteristics.length > 0){
                        this.batteryChar = characteristics[0];
                        seriesCallback(null, 'foo');
                    } else {
              ...
              (seriesCallback) => {
                this.imuService.discoverCharacteristics([this.protocol.services.imuData.IMU_DATA], (error, characteristics) => {
                  if(characteristics.length > 0) {
                    ...
                    this.imuChar.on('read', (data, isNotification) => {
              ...
              (seriesCallback) => {
                this.classifierService.discoverCharacteristics([this.protocol.services.classifier.classifierEvent], (error, characteristics) => {
                  if(characteristics.length > 0) {
                    ...
                    this.classifierChar.on('read', (data, isNotification) => {
              ...
              (seriesCallback) => {
                  this.emgService.discoverCharacteristics([], (error, characteristics) => {
                      if(characteristics.length  == 4) {
                        ...
                        for(let index in characteristics) {
                          ...
                          char.on('read', (data, isNotification) => {
            ...
            ], (error, results) => console.log(`results: ${results}`));
